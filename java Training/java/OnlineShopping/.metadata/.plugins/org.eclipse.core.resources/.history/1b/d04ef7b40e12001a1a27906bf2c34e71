	...
	<div id="templatemo_container">
		...
		<div id="templatemo_content">
			...
			<div id="templatemo_content_right">
				<h1>
					Item's added to <span>${customer.name}'s</span>WishList....
				</h1>
				...
				<table border=2>
					...
					<c:forEach items="${addToCartList1}" var="addToCartList"
						varStatus="status">
							<tr>
								<td>${status.index}</td>
								<td>${addToCartList.productId}</td>
								<td>${productNameList[status.index]}</td>
								<td>${addToCartList.total}</td>
								<td>${addToCartList.quantity}</td>
								<td><a href="OrderController?action=getAllOrderByUserName">Order Now</a></td>
							</tr>
					</c:forEach>
